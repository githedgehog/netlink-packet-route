...
use netlink_packet_core::{
    ...
    Emitable, Nla, NlaBuffer, Parseable,
};
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_BRPORT_STATE => InfoBridgePort::State(
                parse_u8(payload)
                    .context(format!("invalid IFLA_BRPORT_STATE {payload:?}"))?
                    .into(),
            ),
            ...
            IFLA_BRPORT_COST => InfoBridgePort::Cost(
                parse_u32(payload)
                    .context(format!("invalid IFLA_BRPORT_COST {payload:?}"))?,
            ),
            IFLA_BRPORT_MODE => InfoBridgePort::HairpinMode(
                parse_u8(payload)
                    .context(format!("invalid IFLA_BRPORT_MODE {payload:?}"))?
                    > 0,
            ),
            IFLA_BRPORT_GUARD => InfoBridgePort::Guard(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_GUARD {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_PROTECT => InfoBridgePort::Protect(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_PROTECT {payload:?}"
                ))? > 0,
            ),
            ...
            IFLA_BRPORT_LEARNING => InfoBridgePort::Learning(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_LEARNING {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_UNICAST_FLOOD => InfoBridgePort::UnicastFlood(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_UNICAST_FLOOD {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_PROXYARP => InfoBridgePort::ProxyARP(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_PROXYARP {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_PROXYARP_WIFI => InfoBridgePort::ProxyARPWifi(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_PROXYARP_WIFI {payload:?}"
                ))? > 0,
            ),
            ...
            IFLA_BRPORT_ID => InfoBridgePort::PortId(
                parse_u16(payload)
                    .context(format!("invalid IFLA_BRPORT_ID {payload:?}"))?,
            ),
            ...
            IFLA_BRPORT_NEIGH_SUPPRESS => InfoBridgePort::NeighSupress(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_NEIGH_SUPPRESS {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_ISOLATED => InfoBridgePort::Isolated(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_ISOLATED {payload:?}"
                ))? > 0,
            ),
            ...
            IFLA_BRPORT_MRP_RING_OPEN => InfoBridgePort::MrpRingOpen(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_MRP_RING_OPEN {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_MRP_IN_OPEN => InfoBridgePort::MrpInOpen(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_MRP_IN_OPEN {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_MCAST_EHT_HOSTS_LIMIT => {
            ...
            IFLA_BRPORT_LOCKED => InfoBridgePort::Locked(
                parse_u8(payload).context(format!(
                    "invalid IFLA_BRPORT_LOCKED {payload:?}"
                ))? > 0,
            ),
            IFLA_BRPORT_MAB => InfoBridgePort::Mab(
                parse_u8(payload)
                    .context(format!("invalid IFLA_BRPORT_MAB {payload:?}"))?
                    > 0,
            ),
            IFLA_BRPORT_MCAST_N_GROUPS => {
                InfoBridgePort::MulticastNGroups(parse_u32(payload).context(
                    format!("invalid IFLA_BRPORT_MCAST_N_GROUPS {payload:?}"),
                )?)
            }
            IFLA_BRPORT_MCAST_MAX_GROUPS => {
                InfoBridgePort::MulticastMaxGroups(parse_u32(payload).context(
